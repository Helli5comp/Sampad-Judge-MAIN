...
function send() {
	var body = ""
		...
		body += "<h4>" + name + "</h4>";
	body += "<hr>"
		commm = document.getElementById('com').value;
	if (commm.includes("<script")) {
		alert('you can not use script tag');
		return;
	}
	if (commm.includes("<style")) {
		alert('you can not use style tag');
		return;
	}
	if (commm.includes("<link")) {
		alert('you can not use link tag');
		return;
	}
	if (commm.includes("<SCRIPT")) {
		alert('you can not use script tag');
	...
	if (commm == "" || commm == " " || commm == "  " || commm == "   " || commm == "    " || commm == "     " || commm == "      " || commm == "       " || commm == "        " || commm == "         " || commm == "          " || commm == "           " || commm == "            " || commm == "             ") {
